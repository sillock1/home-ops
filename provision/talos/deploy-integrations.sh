...
rm -rf cni/charts
envsubst < ../../../kubernetes/production/infrastructure/kube-system/cilium/app/values.yaml > cni/values.yaml
kustomize build --enable-helm cni | kubectl apply -f -
rm cni/values.yaml
rm -rf cni/charts

rm -rf kubelet-csr-approver/charts
envsubst < ../../../kubernetes/production/infrastructure/system-controllers/kubelet-csr-approver/app/values.yaml > kubelet-csr-approver/values.yaml
